	...
	var handleResponse = function (response) {
		if (response.statusCode === 429) {
			if (response.headers && response.headers["x-ratelimit-reset"]) {
				...
				throw new RateLimitError(response.body, response.statusCode, limitReset, response.body);
			}
			else {
				throw new UnexpectedResponseError(response.body, response.statusCode, response.body);
			}
		}
			...
			throw new UnexpectedResponseError(message, response.statusCode, response.body);
		}
		return response;
	};
